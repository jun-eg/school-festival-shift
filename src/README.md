...
| [`generate.js`](generate.js) | **候補・条件・希望から割り当ての行を組む**（→ 5 の #6・5-5。`生成する` の段の中身）。**踏む 4 段（`generationOrder`）と、目的にしない 4 つ（`generationNotAimed`）と、まとまりで置けない端 5 つ（`runExceptions`）を名前で持つ** | **コア** |
| [`count-violations.js`](count-violations.js) | **置いた人が条件を破っている所を行にする**（→ 5-4。`違反を数える` の段の中身）。**数える 5 つと、数えない ⑥ を名前で持つ** | **コア** |
| [`assignment-grid.js`](assignment-grid.js) | **割り当ての行を、従来のシフト表の形に敷く／その形から戻す**（→ [#213](https://github.com/jun-eg/school-festival-shift/issues/213)）。**行が人・列が 30 分枠・セルが役割名 1 つ**で、**1 日 1 枚**である。**段ではない** — 敷き方だけを持ち、規則を 1 つも持たない。**役割ごとの背景色（`roleColors`）と、違反の行を印を付けるセルに当て戻すのもここである**（`violationCells` → 下の「手直しは、書き換えたその場で数え直す」） | **コア** |
...
| [`shell.js`](shell.js) | **シートを読んで値の表現を揃え、コアを呼び、返ってきた行を書く。**マス目の色を塗り直す（`paintGrids`。背景は役割の色、違反した所は赤い太字）のと、**書き換えを拾って数え直す中身（`recountOnEdit`）もここ**である | **殻** |
...
| **準備日 ／ 学祭1日目 ／ 学祭2日目 ／ 片付け** | 生成の結果。**1 日 1 枚で、行が人・列がその日の 30 分枠・セルが役割名 1 つ**である。**担当者がセルを書き換えるのが仕様である**（→ 5-3） | **書く** | かけない |
...
| **固定（前の周の手直し）が新しい入力と食い違ったときの名指し** | [#156](https://github.com/jun-eg/school-festival-shift/issues/156)（8 の 11）。**ここは書いてあるとおりを数えるだけで、固定として積むのはあちら**である |
...
| [`core.test.mjs`](core.test.mjs) | **スプレッドシートを 1 つも作らずにコアを走らせる。**`SpreadsheetApp` を掴むファイルの一覧／配列を渡して配列が返るか／入っていない段が名指しで返るか／段を差し替えて先に回せるか／**条件が 5-1 の型で段に渡るか**／**表現の揺れ・列数の違い・決めていない種別で止まるか**／**中身が入っている段が未了に出ないか**／**割り当てだけがシート 1 枚に対応しないこと**／**手直しの後の数え直しが生成を通さず、書いたとおりの割り当てから違反と指標を返すか**（手直しで作った規則 1 の違反が出る） | 34 |
...
| [`generate.test.mjs`](generate.test.mjs) | **違反 0 の案が 1 つ出るか**（規則 1・3・4・5 と二重を 1 つずつ見る）**／人数が足りない枠が全部名指しで出るか**（**置いた数 ＋ あと何人 ＝ 需要の合計**）**／同じ入力から同じ案が出るか／外部のソルバーを読んでいないか**（受け入れ条件の 4 つ）**／5-5 の適用の順序どおりに置くか**（きつい順 ／ 同じ枠の中の入れ替え 1 手 ／ 準備・片付けは帯の中 ／ 需要が残っていれば 4 段目が埋める ／ 氏名は空 ／ 固定は見ない ／ 役割名を弾かない）**／まとまりと散らしが入っているか**（**その日に置いた枠が少ない人から採る ／ 1 時間ずつの塊で回る ／ 「連続して入る最小の長さ」を 2 時間にすると塊が 4 枠になる ／ 置けない端 5 つが名前で置いてある**）**／決まらない入力で止まるか／コアの段として繋がっているか。****[`data/前回の希望データ-モック.csv`](../data/前回の希望データ-モック.csv) の 36 人 × 88 枠を通し、違反 0・需要のべ 704 が 置いた 589 ＋ あと何人 115 に揃うことを見る**（**需要は時間帯を明示して全 88 枠に置いている** — **ここは 1 日を 1 本の帯として刻む側なので調理帯が 0 枠で、時間帯を空けた行はどの枠にも立たない** → 5-1 の #2）（**判定そのものは M2 → [#153](https://github.com/jun-eg/school-festival-shift/issues/153)**。**塊と散らしを数えるのは [`scripts/まとまりと散らし.mjs`](../scripts/まとまりと散らし.mjs)**） | 41 |
...
| [`assignment-grid.test.mjs`](assignment-grid.test.mjs) | **従来の形に敷けるか**（行が人・列が 30 分枠・セルが役割名 1 つ）**／行の並びが学籍番号の昇順で、割り当ての順を入れ替えても同じマス目が出るか／敷いて戻すと元の行に戻るか／当てるのが位置ではなく見出しの時刻か／載らないもので名指しして止まるか**（枠に無い時刻・1 セルに 2 役割・学籍番号が空／形式違い・いまの枠に無い見出し・その日の行が無い）**／違反の行が塗るセルに当たるか**（枠 1 つの違反はその枠 ／ 規則 3 は名前のある 2 列 ／ 未充足・別の日・当たらない違反は塗らない ／ 同じセルは 1 回）。**記録がこの形に載るか（→ [`scripts/前回のシフト表.mjs`](../scripts/前回のシフト表.mjs)）はここでは数えない** | 24 |
