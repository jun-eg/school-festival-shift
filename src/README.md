...
| 2 | **役割と必要人数**（`roleNeeds`） | 条件入力「役割と必要人数」 | `{ date, start, end, role, count }` の集合。**日の空欄は「絞らない」、時間帯の空欄はその日の `調理開始`〜`調理終了` の帯である**（→ 5-1 の #2。**枠に当てる読みは `name-unmet.js` の `needCovers` が持つ**） |
...
| 数えるもと | どの枠に効くか | 型 |
| --- | --- | --- |
| **必要人数**（5-1 の #2） | **重なる 30 分枠すべて。**日を空けた行は全日に、時間帯を空けた行はその日の `調理開始`〜`調理終了` の帯に効く | `roleNeeds` |
...
| [`generate.test.mjs`](generate.test.mjs) | **違反 0 の案が 1 つ出るか**（規則 1・3・4・5 と二重を 1 つずつ見る）**／人数が足りない枠が全部名指しで出るか**（**置いた数 ＋ あと何人 ＝ 需要の合計**）**／同じ入力から同じ案が出るか／外部のソルバーを読んでいないか**（受け入れ条件の 4 つ）**／5-5 の適用の順序どおりに置くか**（きつい順 ／ 同じ枠の中の入れ替え 1 手 ／ 準備・片付けは帯の中 ／ 氏名は空 ／ 固定は見ない ／ 役割名を弾かない）**／決まらない入力で止まるか／コアの段として繋がっているか。****[`data/前回の希望データ-モック.csv`](../data/前回の希望データ-モック.csv) の 36 人 × 88 枠を通し、違反 0・需要のべ 704 が 置いた 589 ＋ あと何人 115 に揃うことを見る**（**需要は時間帯を明示して全 88 枠に置いている** — **ここは 1 日を 1 本の帯として刻む側なので調理帯が 0 枠で、時間帯を空けた行はどの枠にも立たない** → 5-1 の #2）（**判定そのものは M2 → [#153](https://github.com/jun-eg/school-festival-shift/issues/153)**） | 38 |
