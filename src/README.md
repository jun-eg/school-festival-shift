...
| [`generate.js`](generate.js) | **候補・条件・希望から割り当ての行を組む**（→ 5 の #6・5-5。`生成する` の段の中身）。**踏む 3 段（`generationOrder`）と、目的にしない 3 つ（`generationNotAimed`）を名前で持つ** | **コア** |
...
| [`generate.test.mjs`](generate.test.mjs) | **違反 0 の案が 1 つ出るか**（規則 1・3・4・5 と二重を 1 つずつ見る）**／人数が足りない枠が全部名指しで出るか**（**置いた数 ＋ あと何人 ＝ 需要の合計**）**／同じ入力から同じ案が出るか／外部のソルバーを読んでいないか**（受け入れ条件の 4 つ）**／5-5 の適用の順序どおりに置くか**（きつい順 ／ 同じ枠の中の入れ替え 1 手 ／ 準備・片付けは帯の中 ／ 需要が残っていれば 4 段目が埋める ／ 氏名は空 ／ 固定は見ない ／ 役割名を弾かない）**／決まらない入力で止まるか／コアの段として繋がっているか。****[`data/前回の希望データ-モック.csv`](../data/前回の希望データ-モック.csv) の 36 人 × 88 枠を通し、違反 0・需要のべ 704 が 置いた 589 ＋ あと何人 115 に揃うことを見る**（**需要は時間帯を明示して全 88 枠に置いている** — **ここは 1 日を 1 本の帯として刻む側なので調理帯が 0 枠で、時間帯を空けた行はどの枠にも立たない** → 5-1 の #2）（**判定そのものは M2 → [#153](https://github.com/jun-eg/school-festival-shift/issues/153)**） | 38 |
...
| [`shell.test.mjs`](shell.test.mjs) | **偽のスプレッドシートの上で殻を走らせる。**`Date` と真偽値と空白が揃うか／**読んだ入力がそのままコアの入口を通るか**／見出しの行を読まず区画ごとに切って読むか／**読み書きが範囲ごとに 1 回か**／**段が欠けているあいだは 1 枚も書かないか**／**構造が崩れていれば 1 行も読まず 1 枚も書かずに止まるか**／**マス目の 4 枚がまとまって「割り当て」1 つに戻るか**／**割り当てがマス目で書かれ、氏名が回答から入るか** | 27 |
