...
**名前の線は 1 本である** — **コードは英語、人に見える言葉と値は日本語のままである**
（→ [#175](https://github.com/jun-eg/school-festival-shift/issues/175) ／
[#178](https://github.com/jun-eg/school-festival-shift/issues/178)）。
**ファイル名・関数名・変数名・オブジェクトのキーと、コメント・JSDoc が英語の側**で、
**シート名・列名・区画の見出し・段の名前・メニューの表示名・例外の文・検査の見出しが日本語の側**である。
**コメントの中で、節の見出し（`6 の #8`）やシート名を引用している所は、引用なので日本語のままである。**
...
3. `src/` の **`.js` 6 つ**を、同じ名前のファイルとして貼る（エディタ上では `.gs` になる）。
   ...
   **関数名も同じで、日本語のままでもメニューから呼べることを実機で見たうえで英語にしてある**
   （→ [#178](https://github.com/jun-eg/school-festival-shift/issues/178)）。
