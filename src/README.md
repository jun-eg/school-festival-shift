...
| [`assignment-grid.js`](assignment-grid.js) | **割り当ての行を、従来のシフト表の形に敷く／その形から戻す**（→ [#213](https://github.com/jun-eg/school-festival-shift/issues/213)）。**行が人・列が 30 分枠・セルが役割名 1 つ**で、**1 日 1 枚**である。**段ではない** — 敷き方だけを持ち、規則を 1 つも持たない。**違反の行を、塗るセルに当て戻すのもここである**（`violationCells` → 下の「手直しは、書き換えたその場で数え直す」） | **コア** |
...
| [`shell.js`](shell.js) | **シートを読んで値の表現を揃え、コアを呼び、返ってきた行を書く。**違反した所をマス目の色で出す（`paintViolations`）のと、**書き換えを拾って数え直す中身（`recountOnEdit`）もここ**である | **殻** |
...
**担当者がマス目のセルを 1 つ書き換えると、検証結果と指標がその場で書き直され、違反した所に色が付く**
...
| **何をするか** | **構造を照らす → 読む → 数え直す → 検証結果と指標を書く → 違反した所を塗る**（→ [`shell.js`](shell.js) の `recountSpreadsheet`）。**生成を走らせない**（→ [`core.js`](core.js) の `recount`）— 1 セル書き換えるたびに残りの枠が組み直されると、**その 1 手で何が変わったかが見えなくなる。****数える段は生成のときと同じものを通す**（数え方を 2 通りに持たない） |
| **マス目** | **書き戻さない。**担当者がいま書いたセルそのものを読んで数える。**規則 1 の違反も、生成は作らないが手直しは作りうる** — **その場合も、そのまま違反として出る**（→ 5 の #13 の ①） |
| **担当者に見えるもの** | **右下の一言**（`数え直した — 違反 N 件 ／ 未充足 M 件`）と、**検証結果・指標の中身**と、**違反した所の色**である |
...
**塗るセルはコアが決め（[`assignment-grid.js`](assignment-grid.js) の `violationCells`）、殻が塗る（[`shell.js`](shell.js) の `paintViolations`）。**
**生成を押したときも同じに塗り直す** — 前の周の色が、違反でなくなったセルに残らない。

| 違反 | 塗る所 |
| --- | --- |
| **規則 1・4・5・同じ枠に二重**（枠 1 つが単位） | **その人の行の、その枠の列のセル** |
| **規則 3**（その人のその日が単位） | **その人の行の、名前のある 2 列**（学籍番号・氏名）。**壊れているのは枠 1 つではない** — **準備にも片付けにも入っていない ④ は、塗る枠そのものが無い** |
| **未充足** | **塗らない。**枠の話であって人の話ではないので、塗る行が無い。**名指しは検証結果である**（→ 上の「割り当ては 4 枚のマス目である」の「空のセル」） |

**マス目の背景色は、この印のためだけに使う。****役割ごとの色はマス目に載せない**
（**前回の配布物 ◎ は役割ごとに色が付いている**が、**配布物は画像のほうである** → [#157](https://github.com/jun-eg/school-festival-shift/issues/157)）。
**同じセルに 2 つの意味の色を重ねると、どちらの色かを担当者が見分けることになる。**
**塗り直すたびに、データの行の背景色を全部消してから塗る** — **担当者が自分で付けた色も消える。**

**読み書きは生成のときと同じ数え方である**（→ 6 の #2 の実装上の注意）。**色は 1 枚につき 2 回で済ませる** — **データの行ぜんぶの色を 1 回で消し、塗るセルを RangeList で 1 回でまとめて塗る。**
**違反した所は、読んだマス目（生成のときは書いたマス目）から引く** — **何行目が誰かを、もう 1 回読みに行かない。**
