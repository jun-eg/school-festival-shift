#!/usr/bin/env node
// 未充足を名指しする側（src/name-unmet.js）の検査。スプレッドシートを作らずに走らせる。
//
...
//   ④ 数えられない需要は、名指しして止まる
//   ⑤ コアの段として繋がり、返った行が検証結果シートの形に合う
//
// 違反は count-violations.test.mjs が見る。
...
// ---- 入力を組む -------------------------------------------------------------
// 値は data/ の転記元と同じ表現で置く。需要を時間帯で絞るのは、出る行を数え切れる数にするためである。
...
/** その日の調理帯（`調理開始`〜`調理終了`）の 30 分枠。時間帯を空けた需要が効く先である。 */
...
/** 30 分に乗らない指定枠（委員会の指定では実際にある → 3 の境界値の表）。 */
...
// 調理帯が 0 枠の日（前回の 2025-11-01 のような準備日）。
check(
