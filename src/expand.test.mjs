#!/usr/bin/env node
// 展開する側（src/expand.js）の検査。スプレッドシートを作らずに走らせる。
//
//   使い方: node src/expand.test.mjs
//
// 見るものは 6 つある（issue #149・#150）。
//   ① 規則 1 の順序を踏む（刻んだ枠を受ける／`,` で区切る／完全に含まれる枠だけ／和集合）
//   ② 3 の「境界値と特別扱い」の 4 つが、そのとおりに出る
//   ③ data/前回の希望データ-モック.csv の 39 人が、記録どおりの件数で展開される
//   ④ 出るのは候補であって割り当てではない（役割が付いていない）
//   ⑤ 決まらない入力は、名指しして止まる
...
// ---- 読み込む ---------------------------------------------------------------
// SpreadsheetApp を文脈に置かない。置かずに通ることもこの検査の一部である。
...
// 4-2 の正規表現と、区間として通す形（→ expand.js の wishIntervalPattern）が一致しているかを見る。
...
// 分は 00-59 まで通る（→ 4-2）ので、`:00` と `:30` に乗らない入力も来る。
const sweep = []
...
// 前回の 5 時刻は記録に無いので、説明文の営業時間から始まりの 4 つを営業開始に、片付け終了を営業終了に置く
// （→ 7 の「前回の 5 時刻の置き方」）。見るのは候補の枠の数なので、帯の内訳は問わない。
...
// 終端 ≤ 始端の区間を持つ人は、そこで止まる。記録では 39 人中 3 人である（→ scripts/件数の期待値.json）。
