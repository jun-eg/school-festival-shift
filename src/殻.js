 ...
 *
 * 他のファイルの値をこのファイルの最上位で使わない（→ コア.js の同じ注意）。
 */
 ...
 * （→ コア.js の 表現を確かめる）。
 *
 * 時刻が HH:MM であることは シート構成.js の注記が決めている（条件入力の「日ごとの営業 4 時刻」）。
...
/** 殻が読むシート。生成しか書かない 2 枚は読まない（→ コア.js の 読まないシート）。 */
function 読むシート() {
  return シートの構成
...
/** 区画の見出しを置くシートは 2 行、置かないシートは 1 行が見出しである（→ テンプレートを組み立てる.js）。 */
function 見出しの行数(構成) {
  return 構成.区画の見出しを置くか ? 2 : 1
}

/**
 * コアに渡す入力を読む。名前は コア.js の 入力の名前 と同じ順で並ぶ。
 ...
 * 何が入っていないかは 未了 が名指しで持っている（→ コア.js の コアの口）。
 */
 ...
 * 手順 はコアの段（→ コア.js の コアの口）で、入っている段だけを渡す。
