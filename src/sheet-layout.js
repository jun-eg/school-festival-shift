 ...
 *
 * ここは定義だけである。SpreadsheetApp を 1 度も掴まない（→ 6 の #8）。
 * 持つ値は、条件入力の初期値（→ 各区画の initialRows）だけである。テンプレートを作るときに区画の入力欄へ置く
 * （→ build-template.js の putInitialRows ／ issue #240 ／ ADR tech-requirements-0015）。
 */
...
const sheetLayout = [
  {
    ...
    // 表現はコアが受け取る形（YYYY-MM-DD ／ HH:MM ／ 数）で持つ。シートに置けば日付と時刻に読まれ、殻が同じ形に戻す。
    frozenRows: 2,
    sections: [
      {
        heading: '日ごとの営業時刻',
