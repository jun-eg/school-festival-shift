...
//   期待値: scripts/件数の期待値.json（期待値と数え方の原本）
//
// 見るものは 3 つ。
//   ① 数え直した値が期待値と一致するか
//   ② その数字が本文（docs/tech-requirements.md ／ data/前回の確定シフト.md）に載っているか（解析はしない）
//   ③ 数え方の宣言を出力の頭に出す
//
// 読むだけ。不一致があれば終了コード 1。
...
function 希望データを数える() {
  ...
  // 参照の解決は大文字・小文字を無視する（→ docs/tech-requirements.md 7）。
