...
//   宣言:   scripts/まとまりと散らしの宣言.json（線と数え方の決めの原本）
//   入力:   scripts/判定の入力.mjs（M2 と同じ置き方）
//
// 数えるのは 2 つ（→ issue #215）。
//   塊   … 同じ人・同じ日で、隣り合う枠に同じ役割が続いた区間。1 件 = 1 つの持ち場
//   散らし … その日に候補枠を持つ人のうち、何人が置かれ、1 人あたり何時間になったか
//
// 違反と未充足は scripts/M2の判定.mjs が判定する（同じ数を 2 か所で判定しない）。
// 例外の名前は src/generate.js の runExceptions をそのまま読む（2 か所に持つと片方が古くなる）。
// 読むだけ。合格の線を外したら終了コード 1。
...
/** 置く前の人（→ src/generate.js）。「その日に置ける人」を見るのに使う。 */
...
/** 時刻が続いているか。枠は帯ごとに刻むので、配列で隣でも飛ぶことがある。 */
function 隣り合う(前, 後) {
 ...
 * null は伸ばせたのに伸びていない端で、線に当てる「名前の付かない短い塊」はこれを数える。
 */
...
/**
 * その日に「置ける人」か（→ docs/tech-requirements.md 5-5）。置く前の人で見る。
 * 規則 3 が見るのは準備・片付けとの向きだけなので、帯に候補が 1 枠も無い人も店の役割に置ける
 * （→ ADR tech-requirements/0017）。置く前の人には準備・片付けが無いので、規則 3 で置けない人は出ない。
 * 見る条件は生成の段と同じにする。
 *   店の役割（1 段目） … 規則 1・4・5 ＋ 規則 3 の向きが合うか
 *   準備・片付け（4 段目） … 規則 1・4・5 ＋ もう片方の帯に入っていないか
 * 混ぜると、準備・片付けしか無い日に「置けないはずの人が置かれている」が出る。
 */
...
/** 例外の名前ごとの件数。塊 1 件は、端 2 つのうち runExceptions の順で先の名前で数える。 */
...
// どの行もちょうど 1 つの塊に入るので、長さの合計は割り当ての行数と揃う（→ 宣言の「塊の切り方」）。
// 揃わなければ数えた先が案と違うので、件数の合否より先に落ちる。
