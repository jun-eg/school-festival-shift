...
//   宣言:   scripts/M1②の宣言.json（合格の線・経路・数えないものの原本）
//
// 数えるのは「変換の途中で人の手が入った箇所」で、合格の線は 0 箇所（→ docs/tech-requirements.md 7 の M1 ②）。
// メニューを押す操作と名簿の画像を選ぶ操作は入力なので数えない。
// src/ の実装をそのまま走らせ、偽にするのは Google 側の器（FormApp ／ SpreadsheetApp ／ Utilities ／ HtmlService）だけ。
// 本物の上で通るかは src/real-device-log.md の項目 10〜12 が持つ。
// 入力は複製したフォームではなく、テンプレートが 4 の定義から作ったフォームの回答スプレッドシート（→ 6 の #6）。
// 読むだけ。人の手が 1 箇所でもあれば終了コード 1。
...
class 偽のフォーム {
  ...
  // 本物と同じく回答先に「フォームの回答 1」を作る（見出しはタイムスタンプ ＋ 設問の題）。
...
// ---- 担当者に向いた口を見張る ------------------------------------------------
// 開いてよい画面は 1 枚（→ 宣言の「画面の枚数」）。ほかに担当者へ聞く口が開いたら、2 の一覧に無い操作。
...
/** モックの 1 行を回答シートに貼った形にする。貼ればタイムスタンプは日時のセルになるので、日時を置く。 */
...
/** 「日ごとの営業時刻」に入れる 4 行（→ 宣言の「入力」）。始まりの 4 つは営業開始。 */
function 営業時刻の行() {
...
function 経路を通す() {
  ...
  // 段 7〜10 — 殻の 1 本をそのまま走らせる。数えるのは型 #6 が出るまでなので、
  // 「展開する」の段を差し替えて取り込んだ型を受け取る（→ core.js の build）。
  try {
    見たもの.notBuilt = run(帳, {
...
// ---- 段ごとに、人の手が入ったかを見る ----------------------------------------
// 返すのは { 運べた, 見たもの }。運べなかった段が「人の手が入った箇所」になる。

const 型に乗る項目 = ['answers', 'canCook', 'grade', 'studentId']

/** 注釈を落とす（注釈の中の名前は呼び出しではない）。src/ に `://` が無いので、// を注釈の始まりと読める。 */
function 注釈を落とす(中身) {
...
/** 宣言が本文から写した値が、いまも本文に載っているかだけを見る。 */
function 本文を見る() {
...
function 主処理() {
  ...
  for (const 段 of 宣言.経路の段) {
    const 見方 = 段の見方[段.鍵]
    if (!見方) throw new Error(`経路の段「${段.鍵}」の見方が無い（宣言と判定の食い違い）`)
    const 結果 = 見方()
