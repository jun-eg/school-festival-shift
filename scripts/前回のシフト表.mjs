...
//   宣言:   scripts/前回のシフト表の宣言.json（決めと期待値の原本）
//
// 見るのは 2 つ。
//   ① 記録（data/ の 4 本）が、行＝人・列＝30 分枠・セル＝役割名 1 つの形にそのまま載るか。
//   ② 配る画像にしても（人・枠・役割）の差分が 0 か（M4「前回の配布物と同じ粒度」→ 5 の #9）。
//
// 敷き方・枠の刻み方・画像の中身は src/ の実装をそのまま使う（書き直した形で一致しても答えにならない）。
// 記録は読むだけ。合格の線を 1 つでも外したら終了コード 1。
...
// ---- 記録を読み、区間を枠に当てる ------------------------------------------
// 区間は重なる枠に当てる（→ 宣言の決め）。記録には 30 分に乗らない区間が 3 行ある。
...
// マス目は 1 セル 1 役割（→ 5 の #8）。記録の重なりはここに出る。
...
行.forEach((一行) => {
  ...
  if (見た[鍵] === 役割) return // 同じ役割の区間が 2 本に割れているだけ
