...
//   宣言:   scripts/M4の宣言.json（合格の線も周の切り方も手直しの選び方も、あちらが持つ。このファイルに書かない）
//
// 数えるのは 3 つである（→ docs/tech-requirements.md 7 の M4 ／ 5 の #11）。
//   ① 再実行の手数 − 初回の 7〜9 の手数
//   ② 再実行で戻ってしまった手直しのセル数（食い違いとして名指しで返されたものは除く）
//   ③ 書き出した画像と、その周のマス目の粒度の差分（前回の配布物と並べた差分は scripts/前回のシフト表.mjs が持つ）
// 合格の線は 3 つとも 0 である。
//
// 実装は書き直さない。src/ の 15 ファイルをそのまま走らせて、担当者が押すのと同じ口
// （menu.js のメニューの関数と onEdit）を呼ぶ。偽で置き換えるのは Google 側の器だけである。
// 本物の上で印が付くか・続けて書き換えたときに落ちないかは、実機の記録が持つ（→ src/real-device-log.md）。
//
// 何も書き換えない。読むだけである。線を 1 つでも外したら終了コード 1 で落ちる。
...
// M1 ② の偽と違うのは、メモを持つことである — 手直しの印はセルのメモである（→ src/assignment-grid.js の fixedNote）。
...
class 偽の範囲 {
  ...
  // 書式だけで、値もメモも動かない（→ src/shell.js の paintGrids ／ writeGrids）。この手は書式を見ない。
...
// ---- 担当者に向いた口を見張る ------------------------------------------------
// 担当者が見るもの・答えさせられるものを全部ここに積む。周ごとに切って数える。
...
/** メニューを表示名で押す（→ menu.js の menuItems）。担当者が押すのは表示名である。 */
...
/** 「2025-10-25 の 24 時」を、回答のタイムスタンプと文字列で比べられる形（翌日の 00:00:00）にする。 */
function 境目の時刻(値) {
...
/** 条件入力の 6 区画を、区画の位置に入れる（→ 2 の一覧 5・6。初回の前に済んでいる → 宣言の「数えないもの」）。 */
function 条件入力を入れる() {
...
/** 回答シートの下に、まだ積まれていない行を積む。入る側がフォームに答えたのと同じである。 */
function 回答を積む(範囲) {
...
/** 条件入力のセルを全部写す。周の前後で比べる（1 つでも動けば、担当者が入れ直したことになる）。 */
function 条件入力の写し() {
  return JSON.stringify([...帳.getSheetByName('条件入力').cells.entries()].sort())
}

// ---- マス目を読む ------------------------------------------------------------
// 判定の側の読みである。行も列も位置で当てず、（学籍番号・見出しの時刻）で当てる（→ 宣言の「1 セルとは」）。
...
/** 宣言の「手直しの選び方」のとおりに選ぶ。まだ印の無い、役割の入ったセルだけに番号を振る。 */
function 手直しを選ぶ() {
...
/**
 * 狙って置く手直し（→ 宣言）。締切の前に出していて、締切と配布のあいだに出し直した人の、
 * 出し直しで希望が 00:00-00:00 になった日のセルを 1 つ、書いてあるとおりに固定する。
 */
function 狙う手直しを選ぶ() {
...
function 周を通す(周, 番号) {
  ...
  // 周のあいだ — 入る側が答える（数えない → 宣言の「周のあいだに誰が何をするか」）。
  ...
  // 届いた回答の人が、生成の結果（指標は 1 枠も置かれない人も 0 で並ぶ → 5-6）に出ているか。
  ...
  // 戻ったセル — 生成の前に印のあったセルを、生成の後のマス目で（シート・学籍番号・時刻）で引く。
  ...
  // 8 — マス目のセルを書き換える（その周の新しい手直し）。初回の後だけ、狙う手直しを 1 つ足す。
  const 選んだ = 手直しを選ぶ()
  if (番号 === 0) {
    見たもの.狙い = 狙う手直しを選ぶ()
  ...
  // 9 — メニュー「画像を書き出す」。ダイアログが開き、描く中身を取りに来る（→ menu.js の exportImagesFromDialog）。
  メニューを押す('画像を書き出す')
  ...
  if (条件入力の写し() !== 条件入力の前) 見たもの.一覧に無い操作.push('条件入力が周の中で動いた（担当者が入れ直したことになる）')
  if ((画面.フォームを作った ?? 0) !== フォームを作った前) 見たもの.一覧に無い操作.push('フォームを作り直した')
  if (見たもの.粒度.差分 > 0) 見たもの.一覧に無い操作.push('画像がこの周のマス目を描いていない（描き直させる手が要る）')

  見たもの.手数 = 見たもの.手.length + 見たもの.一覧に無い操作.length
  return 見たもの
}

/** 画像 4 枚の（名前・時刻・役割）と、いまのマス目の（氏名・時刻・役割）を突き合わせる（→ 宣言の「画像の粒度」）。 */
function 粒度を比べる(画像) {
  const マス目 = マス目を読む()
  const 画像の組 = new Set()
  const マス目の組 = new Set()
...
function 主処理() {
  ...
  // フォームを作り直したかを数えるために、メニュー「フォームを作る」の関数を見張る（押されなければ 0 のまま）。
  ...
  if (狙い.選んだ) {
    ...
    console.log(`  狙って置く手直し: 狙う先が無かった（出し直した人: ${狙い.出し直し.join(' / ') || '無い'}）— 除く道を踏んでいない`)
  }
  console.log('')
  console.log('見ないもの')
