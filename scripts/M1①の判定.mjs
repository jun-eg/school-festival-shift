...
//   宣言:   scripts/M1①の宣言.json（合格の線と営業時刻の置き方の原本）
//
// 数えるのは「型に乗らなかった行」で、合格の線は 0 行（→ docs/tech-requirements.md 7 の M1 ①）。
// 型は src/input-types.js をそのまま走らせる（書き直した型に乗っても答えにならない）。
// 前回の記録が規則を満たすかは見ない（違反を数えるのは src/count-violations.js で、入力は生成した案）。
// 読むだけ。乗らなかった行が 1 行でもあれば終了コード 1。
...
// ---- 型を読む --------------------------------------------------------------
// SpreadsheetApp は置かない（置かずに通ることは src/input-types.test.mjs が見ている）。
...
// 型 #2 と #4 は同じ (日・時間帯・役割名・人数) の行なので、名指しの文が読みやすい #2 で通す。
const 型 = {
  枠: inputTypes[0],
...
/** 確定シフトのモック 4 本を、割り当て 1 行 = 1 要素に開く（在処付き）。 */
function 確定シフトを読む() {
  const 行 = []
...
/** 8:00 のような時刻を HH:MM に揃える。表現だけで、時刻は動かさない。 */
const 時刻を揃える = (文字列) => {
...
/** 始まりの 4 つを営業開始に、片付け終了を営業終了に置き、1 日を 1 本の帯として刻む（→ 宣言の「営業時刻の置き方」）。 */
function 日ごとの枠() {
...
/** 端が接するだけは重なりに数えない（scripts/件数の検算.mjs と同じ向き）。 */
const 重なる = (枠, 区間) => 分(枠.start) < 分(区間.end) && 分(区間.start) < 分(枠.end)

/** 確定シフトの 1 行を 5-1 の 3 つに当てる（→ 宣言の「当て方」）。乗れば null、乗らなければ理由を返す。 */
function 乗らない理由(行, 日ごと) {
  // 日 ＋ 時間帯 ＋ 役割名 — 型 #2／#4 の行になるか。人数は 1（1 行は 1 人ぶん）。
  try {
    toType(型.役割と必要人数, [[行.date, 行.start, 行.end, 行.role, 1]])
  } catch (どこが) {
    return `型 #2／#4 の行にならない — ${型が言ったこと(どこが)}`
  }

  // 学籍番号 — 型 #6 の識別キーの形か。
  if (!studentIdPattern.test(行.memberId)) {
    return `学籍番号が 10 桁の英数字でない（型 #6）— いま: ${行.memberId}`
  }
  ...
  if (重なる枠.length === 0) {
    return `型 #1 の ${行.date} に ${行.start}-${行.end} と重なる枠が無い`
  }
  return null
}

/** 型が名指しした文から、判定では意味を持たない区画と行番号を落とす。 */
const 型が言ったこと = (どこが) => String(どこが.message).replace(/^「[^」]+」の \d+ 行目（シートの \d+ 行目）の?/, '')

/** 端が枠にそのまま分かれるか。分かれない行は乗った側でも内訳に出す。 */
